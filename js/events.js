...
function renderEvents() {
  ...
  const filtered = _events.filter(e => {
    ...
    const tOk = timeFilter === 'upcoming' ? d >= now : timeFilter === 'past' ? d < now : true;
    const yOk = typeFilter === 'All' || e.type === typeFilter;
    return tOk && yOk;
  });
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.addEventListener('click', e => {
    ...
    if (o && !o.contains(e.target) && e.target !== b) {
